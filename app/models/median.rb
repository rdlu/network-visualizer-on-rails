class Median < ActiveRecord::Base
  ...
  def self.calculate (schedule, threshold, reference_date)
    if threshold.goal_method == 'median'
      ...
      median.dsavg = len % 2 == 1 ? results_ordered_by_dsavg[len/2] : (results_ordered_by_dsavg[len/2 - 1] + results_ordered_by_dsavg[len/2]).to_f / 2

      #mediana do upload
      results_ordered_by_sdavg = results.sort_by {|hsh| hsh[:sdavg]}
      median.sdavg = len % 2 == 1 ? results_ordered_by_sdavg[len/2] : (results_ordered_by_sdavg[len/2 - 1] + results_ordered_by_sdavg[len/2]).to_f / 2
